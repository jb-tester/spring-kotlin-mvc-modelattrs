...
class Controller2 {
    ...
    fun multipleAttr(model: Model) {
        model.addAttribute(
            "common_multi_attr1",
            "first of multiple model attributes added via @ModelAttribute-annotated method with Model-type parameter"
        )
        model["common_multi_attr2"] =
            "second of multiple model attributes added via @ModelAttribute-annotated method with Model-type parameter"
    }
    ...
    fun populateCommonModelWithMap(model: Model) {
        ...
        myMap["common_map_attr1"] =
            "first attribute added using addAllAttributes(Map) in @MethodAttribute-annotated method with Model-type parameter"
        myMap["common_map_attr2"] =
            "second attribute added using addAllAttributes(Map) in @MethodAttribute-annotated method with Model-type parameter"
